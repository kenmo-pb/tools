...
; | 2017-07-20 . Version 1.1, added "CenterPreview" positioning option
...
;   You can set the preview to always appear centered over the text, by "CenterPreview = 1"
...
Procedure Show(Color.i)
  ...
  If (Not Shown)
    If (CenterPrev)
      ResizeWindow(0, DispX - (PreviewSize + 2*BorderSize)/2,
          DispY - (PreviewSize + 2*BorderSize),
          #PB_Ignore, #PB_Ignore)
    Else
      ResizeWindow(0,
...
Procedure.i FindRGBValue(Pos.i, *Color.INTEGER)
  If (Pos >= 0)
    ...
    If (LText)
      ...
      While (i)
        Protected j.i = FindString(LText, "RGB", i + 1, #PB_String_NoCase)
        If ((j <= 0) Or (FindString(LText, "(", i) <= j))
            ...
            If (PosInLine >= i) And (PosInLine <= LastParen)
              ...
              If ((CountString(Text, ",") = 2) Or (CountString(Text, ",") = 3))
                *Color\i = RGB(Val(StringField(Text, 1, ",")),
                ...
                If (CenterPrev)
                  DispX = SSM(#SCI_POINTXFROMPOSITION, #Null, LStart + FirstParen - 1)
                  DispX + (SSM(#SCI_POINTXFROMPOSITION, #Null, LStart + LastParen - 1) - DispX)/2
                EndIf
  ...
  Else
    ...
    If (ElapsedMilliseconds() - HoverStart > HoverTime)
      ...
      If (Not Valid)
        ...
        If (Valid And CenterPrev)
          DispX = SSM(#SCI_POINTXFROMPOSITION, #Null, Start)
          DispX + (SSM(#SCI_POINTXFROMPOSITION, #Null, Stop) - DispX)/2
        EndIf
      EndIf
      
      If (Valid)
        If (CenterPrev)
          Protected LN.i = SSM(#SCI_LINEFROMPOSITION, Start) + 1
          DispY = SSM(#SCI_POINTYFROMPOSITION, #Null, Start)
          DispX + R\left
          DispY + R\top
          ; Final positioning
          DispX + 1
...
OpenPreferences(PrefFile)
  ...
  CenterPrev  = ReadPreferenceInteger("CenterPreview", #False)
ClosePreferences()
...
If (CreatePreferences(PrefFile))
  ...
  WritePreferenceInteger("CenterPreview", CenterPrev)
  ClosePreferences()
EndIf
